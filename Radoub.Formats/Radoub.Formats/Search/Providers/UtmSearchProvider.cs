...
public class UtmSearchProvider : SearchProviderBase, IFileSearchProvider
{
    ...
    private static readonly FieldDefinition ResRefField = new() { Name = "ResRef", GffPath = "ResRef", FieldType = SearchFieldType.ResRef, Category = SearchFieldCategory.Identity, Description = "Store resource reference" };
    ...
    public IReadOnlyList<ReplaceResult> Replace(GffFile gffFile, IReadOnlyList<ReplaceOperation> operations)
    {
        ...
        return operations.Select(op => new ReplaceResult
        {
            ...
            Skipped = true, SkipReason = "Replace not yet implemented for UTM provider"
        }).ToList();
    }
}
