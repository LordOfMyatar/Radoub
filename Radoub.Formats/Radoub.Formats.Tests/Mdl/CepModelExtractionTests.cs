...
public class CepModelExtractionTests
{
    ...
    [Theory]
    [InlineData("c_air", "air elemental - not rendering")]
    [InlineData("c_earth", "earth elemental - head missing")]
    [InlineData("c_fairy", "fey pixie green - incomplete")]
    [InlineData("c_cat_lion", "dire tiger? - particularly bad")]
    [InlineData("c_umberhulk", "umber hulk - sideways")]
    [InlineData("c_zomb_rot", "zombie rotting - spike artifact")]
    [InlineData("c_hellhound", "hell hound - spike artifact")]
    [InlineData("c_blinkdog", "blink dog - reference")]
    [InlineData("c_wolf", "wolf - reference (should be OK)")]
    [InlineData("c_bearblck", "black bear - reference")]
    public void AnalyzeProblematicCepModel(string resRef, string description)
    {
        if (!Directory.Exists(HakDir))
        {
            _output.WriteLine("SKIP: HAK directory not found");
        ...
        _output.WriteLine($"=== {resRef}: {description} ===\n");

        // Find in HAK
        var hakData = FindModelInHaks(resRef);
        byte[]? bifData = _gameDataService?.FindBaseResource(resRef, ResourceTypes.Mdl);

        var hasBif = bifData != null && bifData.Length > 0;
        var hasHak = hakData.data != null;

        _output.WriteLine($"BIF: {(hasBif ? $"{bifData!.Length} bytes" : "NOT FOUND")}");
        _output.WriteLine($"HAK: {(hasHak ? $"{hakData.data!.Length} bytes from {hakData.hakFile}" : "NOT FOUND")}");

        // Analyze both versions if available
        var mdlReader = new MdlReader();

        if (hasBif)
        {
            _output.WriteLine("\n--- BIF version ---");
            AnalyzeModel(mdlReader, bifData!, "BIF");
        }

        if (hasHak)
        {
            _output.WriteLine("\n--- HAK version ---");
            AnalyzeModel(mdlReader, hakData.data!, "HAK");
        }

        // Key comparison
        if (hasBif && hasHak)
        {
            var bifIsBinary = bifData!.Length >= 4 && BitConverter.ToUInt32(bifData, 0) == 0;
            var hakIsBinary = hakData.data!.Length >= 4 && BitConverter.ToUInt32(hakData.data, 0) == 0;
            _output.WriteLine($"\nFormat: BIF={FormatLabel(bifIsBinary)}, HAK={FormatLabel(hakIsBinary)}");

            var bifModel = mdlReader.Parse(bifData!);
            var hakModel = mdlReader.Parse(hakData.data!);

            var bifNodes = CountAllNodes(bifModel.GeometryRoot);
            var hakNodes = CountAllNodes(hakModel.GeometryRoot);
            var bifMeshes = CountMeshNodes(bifModel.GeometryRoot);
            var hakMeshes = CountMeshNodes(hakModel.GeometryRoot);
            var bifSkins = CountNodesByType<MdlSkinNode>(bifModel.GeometryRoot);
            var hakSkins = CountNodesByType<MdlSkinNode>(hakModel.GeometryRoot);
            var bifEmitters = CountNodesByType<MdlEmitterNode>(bifModel.GeometryRoot);
            var hakEmitters = CountNodesByType<MdlEmitterNode>(hakModel.GeometryRoot);

            _output.WriteLine($"Nodes: BIF={bifNodes}, HAK={hakNodes}");
            _output.WriteLine($"Meshes: BIF={bifMeshes}, HAK={hakMeshes}");
            _output.WriteLine($"Skins: BIF={bifSkins}, HAK={hakSkins}");
            _output.WriteLine($"Emitters: BIF={bifEmitters}, HAK={hakEmitters}");
            ...
            var bifOnly = bifNames.Except(hakNames, StringComparer.OrdinalIgnoreCase).ToList();
            if (hakOnly.Count > 0)
                _output.WriteLine($"HAK-only nodes: {string.Join(", ", hakOnly.Take(15))}");
            if (bifOnly.Count > 0)
                _output.WriteLine($"BIF-only nodes: {string.Join(", ", bifOnly.Take(15))}");
        }
    }

    private void AnalyzeModel(MdlReader reader, byte[] data, string label)
    {
        var isBinary = data.Length >= 4 && BitConverter.ToUInt32(data, 0) == 0;
        _output.WriteLine($"Format: {FormatLabel(isBinary)}, Size: {data.Length} bytes");
        ...
        try
        {
            ...
            var danglyNodes = CountNodesByType<MdlDanglyNode>(root);
            var emitterNodes = CountNodesByType<MdlEmitterNode>(root);
            var hasNaN = HasNaNVertices(root);
            var hasGarbage = HasGarbageNodeNames(root);

            _output.WriteLine($"Nodes: {totalNodes} total, {meshNodes} mesh, {skinNodes} skin, {danglyNodes} dangly, {emitterNodes} emitter");
            _output.WriteLine($"NaN: {hasNaN}, Garbage names: {hasGarbage}");
            _output.WriteLine($"SuperModel: {(string.IsNullOrEmpty(model.SuperModel) ? "(none)" : model.SuperModel)}");
            _output.WriteLine($"Classification: {model.Classification}");

            // Count total vertices and check for degenerate faces
            int totalVerts = 0, totalFaces = 0, zeroAreaFaces = 0;
            AnalyzeMeshes(root, ref totalVerts, ref totalFaces, ref zeroAreaFaces);
            _output.WriteLine($"Geometry: {totalVerts} vertices, {totalFaces} faces, {zeroAreaFaces} degenerate");

            // Check if model is emitter-heavy (rendering issue)
            if (emitterNodes > 0 && meshNodes == 0)
                _output.WriteLine("⚠ EMITTER-ONLY: No renderable meshes — will appear empty");
            else if (emitterNodes > 0)
                _output.WriteLine($"⚠ Has {emitterNodes} emitters (not rendered)");

            // Check for skin meshes (rendered in bind-pose only)
            if (skinNodes > 0)
                _output.WriteLine($"ℹ Has {skinNodes} skin meshes (bind-pose only, no bone deformation)");
        }
        catch (Exception ex)
        {
            _output.WriteLine($"PARSE FAILED: {ex.GetType().Name}: {ex.Message}");
        }
    }

    private static void AnalyzeMeshes(MdlNode? node, ref int totalVerts, ref int totalFaces, ref int zeroAreaFaces)
    {
        if (node == null) return;
        if (node is MdlTrimeshNode mesh)
        {
            totalVerts += mesh.Vertices?.Length ?? 0;
            if (mesh.Faces != null)
            {
                totalFaces += mesh.Faces.Length;
                foreach (var face in mesh.Faces)
                {
                    // Check for degenerate triangles (two or more identical vertex indices)
                    if (face.VertexIndex0 == face.VertexIndex1 || face.VertexIndex1 == face.VertexIndex2 || face.VertexIndex0 == face.VertexIndex2)
                        zeroAreaFaces++;
                }
            }
        }
        foreach (var child in node.Children)
            AnalyzeMeshes(child, ref totalVerts, ref totalFaces, ref zeroAreaFaces);
    }
