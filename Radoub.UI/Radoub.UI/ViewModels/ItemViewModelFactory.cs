...
public class ItemViewModelFactory
{
    ...
    private string ResolveBaseItemName(int baseItem)
    {
        ...
        if (nameStrRef != null)
        {
            ...
            if (TlkHelper.IsValidTlkString(resolved))
                return resolved!;
        }

        // Fallback to label (format it nicely)
        var label = _gameData.Get2DAValue("baseitems", baseItem, "label");
        if (!string.IsNullOrEmpty(label) && !TlkHelper.IsGarbageLabel(label))
            return TlkHelper.FormatBaseItemLabel(label);

        // Unresolvable — warn and return fallback
        UnifiedLogger.LogApplication(LogLevel.WARN,
            $"Base item type {baseItem} not resolvable (label: {label ?? "null"})");
