...
Write-Host "Parley Clean Rebuild - DEBUG MODE" -ForegroundColor Cyan
Write-Host "================================================" -ForegroundColor Cyan
Write-Host ""
...
Write-Host "[4/5] Restoring and building Parley (Debug)..." -ForegroundColor Yellow
Push-Location $ParleyRoot
try {
    Write-Host "  - Restoring packages..." -ForegroundColor Gray
    dotnet restore Parley.sln --verbosity quiet

    Write-Host "  - Building solution (Debug)..." -ForegroundColor Gray
    $buildOutput = dotnet build Parley.sln --configuration Debug --no-restore 2>&1
...
Write-Host "[5/5] Launching Parley (Debug)..." -ForegroundColor Yellow
$parleyExe = "$ParleyRoot\Parley\bin\Debug\net9.0\Parley.exe"
...
Start-Process -FilePath $parleyExe -WorkingDirectory "$ParleyRoot\Parley\bin\Debug\net9.0"
Write-Host "  ✓ Parley launched" -ForegroundColor Green
Write-Host ""

Write-Host "================================================" -ForegroundColor Cyan
Write-Host "Rebuild complete! Parley is running in Debug mode." -ForegroundColor Cyan
Write-Host "================================================" -ForegroundColor Cyan
