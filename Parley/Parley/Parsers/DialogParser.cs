    ...
    public class DialogParser : GffParser, IDialogParser
    {
        ...
        */
        ...
        private byte[] CreateEnhancedMinimalDlg(Dialog dialog)
        {
            ...
            foreach (var entry in dialog.Entries)
            {
                ...
                if (!string.IsNullOrEmpty(text))
                {
                    ...
                    UnifiedLogger.LogParser(LogLevel.TRACE, $"[CExoLocString] Created {locStringData.Length} bytes for text '{text}' (first 4 bytes: {BitConverter.ToUInt32(locStringData, 0)})");
            ...
            for (int i = 0; i < startCount; i++)
            {
                ...
                UnifiedLogger.LogParser(LogLevel.TRACE, $"🔍 EXPORT DEBUG: Writing start[{i}] with Index={startIndex} (0x{startIndex:X8})");
                ...
                if (startIndex == uint.MaxValue)
                {
                    ...
                    UnifiedLogger.LogParser(LogLevel.TRACE, $"🔧 CORRUPTION FIXED: Start[{i}] Index corrected to {startIndex}");
                }
        ...
        private byte[] WriteCExoLocString(string text)
        {
            ...
            UnifiedLogger.LogParser(LogLevel.TRACE, $"[CExoLocString] Text '{text}' length: {textBytes.Length}");
            UnifiedLogger.LogParser(LogLevel.TRACE, $"[CExoLocString] Calculation: StringRef(4) + StringCount(4) + StringID(4) + StringLength(4) + Text({textBytes.Length}) = {totalSize}");
            UnifiedLogger.LogParser(LogLevel.TRACE, $"[CExoLocString] Expected: 21, Actual: {totalSize}");
        ...
        */
        ...
        private byte[] WriteBinaryGff(List<GffStruct> allStructs, List<GffField> allFields, List<string> allLabels, List<byte> fieldData, string signature, string version, Dialog? dialog = null, List<int>? entryStructIndices = null, List<int>? replyStructIndices = null, List<int>? startStructIndices = null, List<List<int>>? entryPointerStructIndices = null, List<List<int>>? replyPointerStructIndices = null, Dictionary<int, List<int>>? pointerConditionParamsMapping = null, Dictionary<int, List<int>>? nodeActionParamsMapping = null)
        {
            ...
            UnifiedLogger.LogParser(LogLevel.DEBUG, $"Generated DLG binary: {stream.Length} bytes");
            return stream.ToArray();
        }
        ...
        private void UpdateListFieldOffsets(List<GffField> allFields, List<string> allLabels, uint listIndicesOffset, List<GffStruct> allStructs, Dialog dialog)
        {
            ...
            UnifiedLogger.LogParser(LogLevel.TRACE, $"🔧 UpdateListFieldOffsets count comparison:");
            UnifiedLogger.LogParser(LogLevel.TRACE, $"    dialog.Entries.Count = {dialog.Entries.Count}, CountEntryStructs = {entryCount}");
            UnifiedLogger.LogParser(LogLevel.TRACE, $"    dialog.Replies.Count = {dialog.Replies.Count}, CountReplyStructs = {replyCount}");
                    ...
                    if (field.Type == GffField.List)
                    {
                        ...
                        switch (label)
                        {
                            ...
                            case "RepliesList":
                                ...
                                UnifiedLogger.LogParser(LogLevel.TRACE, $"🔧 UpdateListFieldOffsets: RepliesList[{currentEntryFieldIndex}] {oldOffset} → {field.DataOrDataOffset}");
                            ...
                            case "EntriesList":
                                ...
                                UnifiedLogger.LogParser(LogLevel.TRACE, $"🔧 UpdateListFieldOffsets: EntriesList[{currentReplyFieldIndex}] {oldOffsetE} → {field.DataOrDataOffset}");
            ...
            UnifiedLogger.LogParser(LogLevel.TRACE, $"🔧 UpdateListFieldOffsets SUMMARY: Updated {repliesListUpdated} RepliesList fields, {entriesListUpdated} EntriesList fields");
        }
        ...
        private uint CalculateRepliesListOffset(uint individualListsOffset, Dialog dialog, int entryIndex)
        {
            ...
            UnifiedLogger.LogParser(LogLevel.TRACE, $"🔧 CalculateRepliesListOffset: Entry[{entryIndex}] offset = {offset} (relative to individualListsOffset {individualListsOffset})");
            return offset;
        }
        ...
        private uint CalculateEntriesListOffset(uint individualListsOffset, Dialog dialog, int replyIndex)
        {
            ...
            UnifiedLogger.LogParser(LogLevel.TRACE, $"🔧 CalculateEntriesListOffset: Reply[{replyIndex}] offset = {offset} (relative to individualListsOffset {individualListsOffset})");
            return offset;
        }
